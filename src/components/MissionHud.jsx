...
export default function MissionHud() {
    ...
    const metrics = [
        {
            label: 'Flights',
            value: layers.aircraft.count,
            pct: Math.max(8, Math.min(100, Math.round((layers.aircraft.count / 5000) * 100))),
            color: '#00b4ff',
        },
        {
            label: 'Satellites',
            value: layers.satellites.count,
            pct: Math.max(8, Math.min(100, Math.round((layers.satellites.count / 4500) * 100))),
            color: '#ffaa00',
        },
        {
            label: 'CCTV',
            value: layers.cctv.count,
            pct: Math.max(8, Math.min(100, Math.round((layers.cctv.count / 6500) * 100))),
            color: '#00ff41',
        },
        {
            label: 'Traffic',
            value: layers.traffic.count,
            pct: Math.max(8, Math.min(100, Math.round((layers.traffic.count / 500) * 100))),
            color: '#ff69b4',
        },
    ];
    ...
    return (
        <>
            ...
            >
                {metrics.map((metric) => (
                    <div key={metric.label} className="mission-metric">
